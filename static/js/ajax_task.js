...
var pageAction = {
	...
	init: function(settings) {
		...
		pageAction.taskNotification();
		pageAction.showNewNotification();
		...
		$(pageAction.config.taskSubmitButton).live("click", function() {
			...
			if (task.trim() !== '' && tag.trim() !== '') {
				...
				$.ajax({
					...
					pageAction.insertNewTask(data, function() {
						pageAction.config.socket.send({
							'id': pageAction.config.notificationId
						});
					});
				});
			} else {
	...
	insertNewTask: function(data, callback) {
		if (data) {
			...
			pageInit.randomCategory();
			$(pageInit.config.taskCreatorPanel).hide();
			pageAction.config.notificationId = data.id;
		}
	...
	taskNotification: function() {
		pageAction.config.socket = new io.Socket();
		pageAction.config.socket.connect();
		pageAction.config.socket.on('connect', function() {
			pageAction.config.socket.subscribe('notification');
		});
		pageAction.config.socket.on('message', pageAction.countNewNotification);
	},

	countNewNotification: function(data) {
		pageAction.config.notificationPool.push(data.id);
		console.log(pageAction.config.notificationPool);
		$(pageAction.config.notificationBar)
			.slideDown()
			.find("span")
			.html(String(pageAction.config.notificationPool.length));
	},

	showNewNotification: function() {
		$(pageAction.config.notificationBar).live("click", function() {
			$(pageAction.config.notificationBar).slideUp();
			$.ajax({
				type: "POST",
				url: window.location.href,
				data: {
					"new_notify_ids": pageAction.config.notificationPool
				},
				traditional: true,
				cache: false,
				dataType: "json"
			...
			.done(function(data) {
				$(data).each(function(i) {
					pageAction.insertNewTask(data[i]);
				});
				pageAction.config.notificationPool = [];
			});	
		})
	},
